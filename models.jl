...
function off_buffer_update!(off_surprise_v, off_choice_v, off_dr_v, surprise, choice, dr)
	...
	if surprise > min_surprise
		...
		off_choice_v[min_idx] = choice
		off_dr_v[min_idx] = dr
	end
end


function off_value_update!(off_surprise_v, off_choice_v, off_dr, b1_m, b2_m, n, decay, n_off_samples, rng, session_idx)
	...
	for s in rand(rng, d, n_off_samples)
		...
		if off_choice_v[s] == 1
			...
			b2_m[i, session_idx] = b2_m[i - 1, session_idx] * decay

		elseif off_choice_v[s] == 2

			b2_m[i, session_idx] = b2_m[i - 1, session_idx] + n * (off_dr / n_off_samples - b2_m[i - 1, session_idx])

			b1_m[i, session_idx] = b1_m[i - 1, session_idx] * decay
...
function RW_update!(r1, r2, r1_exp, r2_exp, n, beta, decay, rng, b1 = 0.0, b2 = 0.0)
	...
	return (r1_updated, r2_updated, choice, r, dr)
end
...
function prob_RW_update!(r1, r2, r1_exp, r2_exp, n, m, s, beta, decay, rng, b1 = 0.0, b2 = 0.0)
	...
	return (r1_updated, r2_updated, choice, r, dr, surprise)
end
...
function KF_update!(r1, r2, r1_exp, r2_exp, s_r1, s_r2, 
	...
	return (r1_updated, r2_updated, s_r1_updated, s_r2_updated, choice, r, dr)
end
